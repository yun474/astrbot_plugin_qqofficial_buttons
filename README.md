...
工作台里可以新建菜单、编辑 Markdown 正文、图片、布局、动作和自定义指令。保存后，在 QQ 会话发送：

```text
/按钮 starter_menu
```

也可以在菜单的“自定义指令”里一行配置一个触发词，例如 `/导航`。这些触发词须以 `/` 开头，并使用机器人当前的唤醒前缀设置；若与其他插件指令同名，可能由多个插件同时处理，请换一个名称。

图片字段会在 Markdown 消息尾部加入 QQ 支持的图片语法；也可以直接在正文里写 Markdown 图片。预览是浏览器侧的近似渲染，最终效果以 QQ 客户端为准。图片地址需要公网可访问的 HTTPS URL，并由你保持长期有效。没有原生 Markdown 权限时，`auto` 模式会退回普通正文并附上图片链接，无法在同一条菜单消息中展示该图。
